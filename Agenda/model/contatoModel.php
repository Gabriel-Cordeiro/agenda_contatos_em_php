...
class contato {
	...
	function insertContato($novoCliente)
    {
        ...
        $verifica = mysqli_query($connect->myCon,"insert into contatos (id_usuario,nome,email,sobrenome,telefone,
        endereco,bairro,cidade,cep)
        values ( '$novoCliente->id_usuario','$novoCliente->nome','$novoCliente->email','$novoCliente->sobrenome','
        $novoCliente->telefone','$novoCliente->endereco','$novoCliente->bairro','$novoCliente->cidade',
        '$novoCliente->cep')");
    ...
    function pegarContatosPorIdUsuario($id_usuario)
    {
        ...
        $resultado = mysqli_query($connect->myCon,"SELECT * FROM contatos WHERE id_usuario = '$id_usuario'") 
